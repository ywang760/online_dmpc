...
for root, dirs, files in os.walk(stats_dir_name):
    for subdir in dirs:
        ...
        stats_file = [
            file for file in os.listdir(subdir_path) if file.startswith(f"stats")
        ][0]
...
success_rate, success_rate_ci = CI_compute(np.array(success_rates))

print(f"Stats are averaged across {len(run_times_all)} experiments")
